...
Get-Childitem -Recurse -Include *.docx,*.pdf,*.xlsx -Path '.\Week 13\Homework\Documents' | export-csv `
-Path '.\Week 13\Homework\files.csv'

# Import CSV 
$fileList = Import-Csv -Path '.\Week 13\Homework\files.csv'

# For each file in the CSV
foreach ($f in $fileList) {
    
    # Encrypt every single file within the CSV file
    Invoke-AESEncryption -Mode Encrypt -key "MySuperSecurePassword" -Path $f.FullName
